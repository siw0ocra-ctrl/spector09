...
function updateGaussBursts(dt){if(!run||run.paused||run.ended)return;const pending=[];for(const burst of run.gaussBursts){burst.delay-=dt;if(burst.delay>1e-9){pending.push(burst);continue}for(let i=0;i<burst.count;i++)fire(burst.angle+(i-(burst.count-1)/2)*.07,burst.damage,burst.color,{weaponId:'gauss',followup:true});beep(245,.025)}run.gaussBursts=pending}
function weaponTick(dt){updateGaussBursts(dt);updateMines(dt);updateDrones(dt);let r=run,targets=()=>combatTargets(),nearest=nearestTarget(targets(),r.x,r.y);r.visualAim=nearest?Math.atan2(nearest.y-r.y,nearest.x-r.x):null;for(const id of Object.keys(r.weapons)){if(id==='drone')continue;if(r.ended)return;r.cooldowns[id]=(r.cooldowns[id]||0)-dt;if(r.cooldowns[id]>0)continue;if(id==='shotgun'){const st=weaponStats(id);plantMine(st);r.cooldowns[id]=(1.9-st.level*.12)*st.cooldown;continue}if(!nearest)continue;let w=weapons.find(w=>w.id===id),st=weaponStats(id),a=Math.atan2(nearest.y-r.y,nearest.x-r.x),d=st.damage,L=st.level,A=st.aw,area=st.area,cd=1;r.angle=a;
if(id==='gauss'){const count=L>=5?3:L>=3?2:1,scale=count===3?.5:count===2?.65:1;for(let i=0;i<count;i++)fire(a+(i-(count-1)/2)*.07,d*scale,w.color,{weaponId:id});if(A)r.gaussBursts.push({delay:.09,angle:a,count,damage:d*scale*.6,color:w.color});cd=.56/(1+(L-1)*.08);beep(200,.03)}
...
function weaponDetails(id){
 ...
 if(id==='gauss'){const n=L>=5?3:L>=3?2:1;return `${n}발 · 발당 ${damage(n===3?.5:n===2?.65:1)} 피해 · ${seconds(.56/(1+(L-1)*.08))}초 간격 · 탄환당 1명 적중${A?" · 0.09초 뒤 후속 "+n+"발 · 발당 "+damage((n===3?.5:n===2?.65:1)*.6)+" 피해 (60%)":""}`}
