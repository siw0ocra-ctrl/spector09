...
const weapons=[
{id:'gauss',name:'가우스 소총',icon:'AR',desc:'가장 가까운 적에게 고속 탄환 발사',passive:'rapid',awake:'오버드라이브',effect:'3연발 관통탄 · 사격 속도 증가',base:90,color:'#def49b'},
{id:'shotgun',name:'산탄총',icon:'SG',desc:'전방으로 퍼지는 다중 산탄',passive:'power',awake:'헬파이어',effect:'산탄 2배 · 폭발 탄두',base:100,color:'#f5bf77'},
...
function resize(){const rect=canvas.getBoundingClientRect();W=Math.max(1,rect.width);H=Math.max(1,rect.height);canvas.width=W;canvas.height=H;ctx.imageSmoothingEnabled=false}
...
function hurt(e,damage,weaponId){if(run.ended||e.dead)return;if(e.isChest){breakChest(e);return;}if(Math.random()<criticalChance()){damage*=2;run.fx.push({x:e.x,y:e.y,r:e.r,max:e.r*1.8,t:.18,life:.18,color:'#ffe180'})}if(weaponId&&run.weapons[weaponId])run.weaponDamage[weaponId]=(run.weaponDamage[weaponId]||0)+Math.min(Math.max(0,e.hp),damage);e.hp-=damage;e.hit=.08;if(e.hp<=0&&!e.dead){e.dead=true;run.kills++;run.killTypes[e.type]++;let gold=Math.max(1,Math.floor(([2,4,7,50][e.type])*(1+save.upgrades.gold*.1)*modeRules().gold));run.gold+=gold;run.orbs.push({x:e.x,y:e.y,n:[2,4,7,25][e.type]*modeRules().xp});run.fx.push({x:e.x,y:e.y,r:5,max:e.r*2,t:.25,life:.25,color:e.type===3?'#f4b273':'#bcc978'});if(e.type===3)end(true)}}
...
for(let e of r.enemies){if(e.dead)continue;if(e.type===3){updateBoss(e,dt);if(r.ended)return;continue}let dx=r.x-e.x,dy=r.y-e.y,d=Math.hypot(dx,dy)||1;e.angle=Math.atan2(dy,dx);let stop=e.type===1&&d<260;if(!stop){e.x+=dx/d*e.speed*(e.slow>r.t?.45:1)*dt;e.y+=dy/d*e.speed*dt}e.hit=Math.max(0,e.hit-dt);if(d<e.r+13)damagePlayer((e.type===3?26:11)*(1+r.stage*.13));if(e.type===1||e.type===3){e.shot-=dt;if(e.shot<0&&d<700){let n=e.type===3?9+Math.max(0,r.stage-4)*3:1;for(let i=0;i<n;i++){let a=e.type===3?i*Math.PI*2/n+r.t*.4:e.angle;r.hostile.push({x:e.x,y:e.y,vx:Math.cos(a)*145*modeRules().projectile,vy:Math.sin(a)*145*modeRules().projectile,life:5,r:6})}e.shot=e.type===3?Math.max(1.3,2.3-Math.max(0,r.stage-4)*.25):3.2}}if(d>Math.hypot(W,H)+300&&e.type!==3){let a=Math.random()*Math.PI*2;e.x=r.x+Math.cos(a)*(Math.hypot(W,H)/2+50);e.y=r.y+Math.sin(a)*(Math.hypot(W,H)/2+50)}}
for(let b of r.bullets){if(b.homing){let target=combatTargets().sort((a,c)=>Math.hypot(a.x-b.x,a.y-b.y)-Math.hypot(c.x-b.x,c.y-b.y))[0];if(target){let a=Math.atan2(target.y-b.y,target.x-b.x);b.vx=Math.cos(a)*320;b.vy=Math.sin(a)*320}}b.x+=b.vx*dt;b.y+=b.vy*dt;b.life-=dt;for(let e of combatTargets()){if(!e.dead&&!b.hit.has(e)&&Math.hypot(b.x-e.x,b.y-e.y)<e.r+b.r){b.hit.add(e);if(b.blast){ring(b.x,b.y,b.blast,b.color);for(let target of combatTargets())if(!target.dead&&Math.hypot(target.x-b.x,target.y-b.y)<b.blast)hurt(target,b.damage,b.weaponId)}else hurt(e,b.damage,b.weaponId);if(--b.pierce<=0){b.life=0;break}}}if(r.ended)return;}
...
if(r.xp>=r.next&&!r.ended){r.xp-=r.next;r.level++;r.next=Math.floor(10+r.level*2.2);levelUp()}updateHUD()}
function draw(){if(!run||view!=='battle')return;let r=run;ground(ctx,W,H,r.x,r.y,stages[r.stage],r.t);let sx=x=>x-r.x+W/2,sy=y=>y-r.y+H/2;drawHazards(sx,sy);drawBossWarnings(sx,sy);drawChests(sx,sy);drawDrops(sx,sy);if(r.stage===3&&r.t>25&&!r.bossSpawned&&r.t%12<4){ctx.fillStyle='#a4d13e38';ctx.beginPath();ctx.arc(sx(Math.sin(Math.floor(r.t/12)*7)*250),sy(Math.cos(Math.floor(r.t/12)*5)*250),75,0,7);ctx.fill()}
for(let o of r.orbs){let x=sx(o.x),y=sy(o.y);ctx.fillStyle='#75c2d2';ctx.beginPath();ctx.moveTo(x,y-5);ctx.lineTo(x+4,y);ctx.lineTo(x,y+5);ctx.lineTo(x-4,y);ctx.closePath();ctx.fill()}
for(let e of r.enemies){let x=sx(e.x),y=sy(e.y);if(x< -100||x>W+100||y< -100||y>H+100)continue;ctx.fillStyle='#00000030';ctx.beginPath();ctx.ellipse(x,y+e.r*.7,e.r*.8,e.r*.35,0,0,7);ctx.fill();if(e.type===3)drawBoss(ctx,r.stage,x,y,e.r,e.angle,r.t,e.hit);else drawMonster(ctx,e.type,x,y,e.r,e.angle,r.t,e.hit);if(e.hp<e.maxHp){ctx.fillStyle='#342f29';ctx.fillRect(x-e.r,y-e.r-13,e.r*2,3);ctx.fillStyle='#ed956d';ctx.fillRect(x-e.r,y-e.r-13,e.r*2*Math.max(0,e.hp/e.maxHp),3)}}
for(let b of r.bullets){ctx.strokeStyle=b.color||'#e5f2ad';ctx.lineWidth=b.type?4:2;ctx.beginPath();ctx.moveTo(sx(b.x),sy(b.y));ctx.lineTo(sx(b.x-b.vx*.018),sy(b.y-b.vy*.018));ctx.stroke();const skin=equippedCosmetic('trail');if(skin)drawBulletCosmetic(ctx,skin,sx(b.x),sy(b.y),Math.atan2(b.vy,b.vx),r.t)}
for(let b of r.hostile){ctx.fillStyle=b.color||'#ed956d';ctx.beginPath();ctx.arc(sx(b.x),sy(b.y),5,0,7);ctx.fill()}
ctx.strokeStyle=r.stim>0?'#d7ff85':equippedCosmetic('suit')?.color||'#c7f26850';ctx.lineWidth=1;ctx.beginPath();ctx.ellipse(W/2,H/2+13,22,10,0,0,7);ctx.stroke();if(r.damageCD>0)ctx.globalAlpha=.5+Math.sin(r.t*45)*.25;ctx.save();if(equippedCosmetic('suit'))ctx.filter='hue-rotate('+equippedCosmetic('suit').hue+'deg)';sprite(ctx,'player',W/2,H/2,46,r.angle);ctx.restore();ctx.globalAlpha=1;
drawLootText(sx,sy);drawWeaponEffects(sx,sy);for(let f of r.fx){if(f.line)continue;let t=1-f.t/f.life;ctx.strokeStyle=f.color;ctx.globalAlpha=1-t;ctx.lineWidth=3;ctx.beginPath();ctx.arc(sx(f.x),sy(f.y),f.r+(f.max-f.r)*t,0,7);ctx.stroke();ctx.globalAlpha=1}if(r.damageCD>.4){ctx.fillStyle='#d14e2517';ctx.fillRect(0,0,W,H)}}
function updateHUD(){let r=run;$('timer').textContent=String(Math.floor(r.t/60)).padStart(2,'0')+':'+String(Math.floor(r.t%60)).padStart(2,'0');$('hp').max=r.maxHp;$('hp').value=r.hp;$('hpText').textContent=Math.ceil(r.hp)+' / '+r.maxHp;$('xp').max=r.next;$('xp').value=r.xp;$('level').textContent='LV.'+r.level;$('kills').textContent=r.kills+' 처치';$('runGold').textContent='예상 ◆ '+r.gold+' G';$('stim').innerHTML=r.stim>0?'가속 중 <b>'+r.stim.toFixed(1)+'s</b>':r.stimCD>0?'재충전 <b>'+Math.ceil(r.stimCD)+'s</b>':'SPACE <b>스팀팩</b>';$('stim').disabled=r.stimCD>0;let boss=r.enemies.find(e=>e.type===3);if(boss){$('bossHp').max=boss.maxHp;$('bossHp').value=Math.max(0,boss.hp)}renderLoadout()}

let last=0;function frame(t){let dt=Math.min(.035,(t-last)/1000||.016);last=t;tick(dt);draw();requestAnimationFrame(frame)}requestAnimationFrame(frame);
...
$('creditsBtn').onclick=()=>{if(view==='battle')pause();else{openModal('<div class="eyebrow">ASSET CREDITS</div><h2>함께 만든 우주.</h2><p>게임 그래픽: <a href="https://kenney.nl/assets" target="_blank" rel="noopener">Kenney</a> · CC0<br>드론: <a href="https://kenney.nl/assets/space-shooter-remastered" target="_blank" rel="noopener">Space Shooter Remastered</a> · CC0<br>효과음: 브라우저 합성 오디오<br>게임 디자인 및 코드: SECTOR 09</p><p>영구 강화와 골드는 서버에 저장됩니다. 다른 브라우저에서 이어 하려면 내 데이터의 복구 코드를 보관하세요.</p><button id="closeCredits">닫기</button>');$('closeCredits').onclick=closeModal}};
...
function buyWeapon(){}
function renderShop(){for(const [id,on] of [['researchTab',armoryTab==='research'],['weaponsTab',armoryTab==='weapons']])$(id).setAttribute('aria-selected',String(on));if(armoryTab==='research'){renderResearch();return}$('upgrades').innerHTML=weapons.map(w=>{let lv=save.weaponResearch[w.id],p=passives.find(p=>p.id===w.passive);return `<article class="upgrade weapon-research"><div class="upgrade-top"><span>${w.icon}</span><span>연구 ${lv} / 10</span></div><h3>${w.name}</h3><p>${w.desc}<br>현재 피해 +${lv*8}% · 재사용 시간 −${lv*2}%</p><div class="recipe"><b>✦ ${w.awake}</b><small>${w.name} Lv.5 + ${p.name} Lv.3</small><small>${w.effect}</small></div><p class="research-note">연구 1단계: 피해 +8%, 재사용 시간 −2%<br>전투 레벨은 매 작전 1부터 성장합니다.</p><button data-weapon="${w.id}" ${lv===10||save.gold<weaponCost(w)?'disabled':''}>${lv===10?'최대 연구 완료':'◆ '+weaponCost(w)+' G · 연구'}</button></article>`}).join('');$('upgrades').querySelectorAll('button').forEach(b=>b.onclick=()=>{buyWeapon(b.dataset.weapon)})}
...
function choose(id){if(!run||run.ended||run.modalMode!=='choice'||!run.choices.includes(id))throw Error('현재 선택할 수 없는 강화입니다.');const tier=run.choiceRarities[run.choices.indexOf(id)]??0,m=rarities[tier].mult;if(id==='gold'){run.gold+=25;run.goldSupplies++}else if(id==='heal')run.hp=Math.min(run.maxHp,run.hp+50);else{const [kind,key]=id.split(':');if(kind==='w'){if(!run.weapons[key]&&Object.keys(run.weapons).length>=modeRules().slots)throw Error('무기 슬롯이 가득 찼습니다.');run.weaponBonuses[key]=(run.weaponBonuses[key]||0)+.28*(m-1);run.weapons[key]=(run.weapons[key]||0)+1}else{run.passiveBonuses[key]=(run.passiveBonuses[key]||0)+(m-1);run.skills[key]++;if(key==='regen')run.hp=Math.min(run.maxHp,run.hp+Math.ceil(35*m));if(key==='armor'){run.maxHp+=20*m;run.hp+=20*m}if(key==='magnet')run.speed*=1+.04*m}run.cardHistory.push({id,tier})}checkAwakenings();run.paused=false;run.modalMode=null;run.choices=[];closeModal();updateHUD();canvas.focus()}
...
function statsHTML(){let r=run,rows=[['치명타 확률',Number((criticalChance()*100).toFixed(2))+'%'],['치명타 피해','200%'],['체력',Math.ceil(r.hp)+' / '+r.maxHp],['공격력 증가','+'+Math.round(((1+save.upgrades.damage*.12)*(1+passiveStrength('power')*.18)-1)*100)+'%'],['이동 속도',(r.speed*(r.stim>0?1.55:1)).toFixed(0)],['피해 감소',Number((passiveStrength('armor')*8).toFixed(2))+'%'],['초당 회복',(save.upgrades.regen*.3+passiveStrength('regen')*.4).toFixed(2)],['범위 증가','+'+Number((passiveStrength('area')*15).toFixed(2))+'%'],['수집 범위',Math.round(75*(1+save.upgrades.magnet*.2+passiveStrength('magnet')*.35))],['골드 증가','+'+(save.upgrades.gold*10)+'%'],['공통 재사용 시간','−'+Math.round((1-Math.pow(.9,passiveStrength('rapid')))*100)+'%'],['레벨 / 처치',r.level+' / '+r.kills],['카드별 무료 리롤',r.cardRerolls.join(' / ')+'회'],['공용 리롤',r.rerolls+' / '+save.upgrades.reroll+'회'],['보급 상자',r.chestsOpened+'개 개봉']];return `<div class="stat-grid">${rows.map(([k,v])=>`<div><small>${k}</small><b>${v}</b></div>`).join('')}</div><p>보급 상자를 한 번 공격하면 아이템이 떨어집니다. 떨어진 아이템에 다가가면 획득합니다. 골드 80% · 자석 10% · 회복 10%. 자석은 맵의 경험치를 회수하고, 회복은 최대 체력의 30%를 채웁니다.</p><h3>장착 무기 · 각성 조건</h3><div class="pause-weapons">${Object.keys(r.weapons).map(id=>{let w=weapons.find(w=>w.id===id),p=passives.find(p=>p.id===w.passive),st=weaponStats(id);return `<div><b>${st.aw?'✦ '+w.awake:w.name} · Lv.${st.level}/5</b><small>${p.name} ${r.skills[p.id]}/5 · 각성 조건 3 · ${st.aw?'각성 완료':w.awake+' 각성 대기'}</small><small>기준 탄환 피해 ${st.damage.toFixed(1)} · 연구 ${save.weaponResearch[id]}/10 · ${w.desc}</small><small>획득 카드: ${run.cardHistory.filter(c=>c.id==='w:'+id).map(c=>rarities[c.tier].name).join(' / ')||'시작 무기'}</small></div>`}).join('')}</div><div class="passive-history">${passives.filter(p=>run.skills[p.id]).map(p=>`<p>${p.name} Lv.${run.skills[p.id]}/5 · ${run.cardHistory.filter(c=>c.id==='p:'+p.id).map(c=>rarities[c.tier].name).join(' / ')}</p>`).join('')}</div><p>무기별 피해 배율·발사 수는 공격 방식에 따라 적용됩니다. 무기 연구와 스팀팩의 재사용 시간 효과는 추가 적용됩니다.</p>`}
function resume(){if(run?.ended||run?.modalMode!=='pause')return;run.paused=false;run.modalMode=null;closeModal();canvas.focus()}
function pause(){if(!run||run.ended||run.paused)return;run.paused=true;run.modalMode='pause';keys={};joy={x:0,y:0};openModal(`<div class="eyebrow">PAUSED / CURRENT STATS</div><h2>현재 능력치</h2>${statsHTML()}<div class="modal-actions"><button id="resume" class="primary">전투 복귀 · ESC</button><button id="retreat">골드 챙기고 귀환</button></div>`);$('resume').onclick=resume;$('retreat').onclick=()=>end(false,true)}
function renderLoadout(){let ids=Object.keys(run.weapons),glyphs={gauss:'🔫',shotgun:'💥',laser:'🔦',missile:'🚀',nova:'🌀',drone:'🛰️',flame:'🔥',lightning:'⚡'};let html=Array.from({length:modeRules().slots},(_,i)=>{let w=weapons.find(w=>w.id===ids[i]);if(!w)return `<div class="weapon-slot empty" role="img" aria-label="빈 무기 슬롯 ${i+1}"></div>`;let aw=run.awakened[w.id],p=passives.find(p=>p.id===w.passive),label=`${aw?w.awake:w.name} · 무기 ${run.weapons[w.id]}/5 · ${p.name} ${run.skills[p.id]}/5 (각성 3)${aw?' · 각성 완료':''} · 능력치 보기`;return `<button class="weapon-slot ${aw?'awakened':''}" aria-label="${label}" title="${label}"><span aria-hidden="true">${w.id==='drone'?'<img src="assets/drone.png" alt="" width="25" height="25">':glyphs[w.id]}</span></button>`}).join('');if($('loadout').innerHTML!==html){$('loadout').innerHTML=html;$('loadout').querySelectorAll('button').forEach(b=>b.onclick=pause)}}
...
function weaponTick(dt){updateDrones(dt);let r=run,targets=()=>combatTargets(),nearest=targets().sort((a,b)=>Math.hypot(a.x-r.x,a.y-r.y)-Math.hypot(b.x-r.x,b.y-r.y))[0];for(const id of Object.keys(r.weapons)){if(id==='drone')continue;if(r.ended)return;r.cooldowns[id]=(r.cooldowns[id]||0)-dt;if(r.cooldowns[id]>0||!nearest)continue;let w=weapons.find(w=>w.id===id),st=weaponStats(id),a=Math.atan2(nearest.y-r.y,nearest.x-r.x),d=st.damage,L=st.level,A=st.aw,area=st.area,cd=1;r.angle=a;
if(id==='gauss'){let count=A?3:1;for(let i=0;i<count;i++)fire(a+(i-(count-1)/2)*.07,d,w.color,{weaponId:id,pierce:A?3:1});cd=.56/(1+(L-1)*.08);beep(200,.03)}
if(id==='shotgun'){let count=3+L+(A?6:0);for(let i=0;i<count;i++)fire(a+(i-(count-1)/2)*.1,d*.55,w.color,{weaponId:id,life:.55*area,blast:A?28:0});cd=1.4}
...
if(id==='nova'){let rad=(90+L*15)*(A?1.6:1)*area;ring(r.x,r.y,rad,w.color);for(let e of targets())if(Math.hypot(e.x-r.x,e.y-r.y)<rad){hurt(e,d*1.5,id);if(A)e.slow=r.t+2}cd=3.6-L*.2}
if(id==='laser'){for(let k=0;k<(A?3:1);k++){let ang=a+(A?(k-1)*.3:0),range=550*area*(A?1.3:1);beam(r.x,r.y,r.x+Math.cos(ang)*range,r.y+Math.sin(ang)*range,w.color,4+L);for(let e of targets()){let dx=e.x-r.x,dy=e.y-r.y,along=dx*Math.cos(ang)+dy*Math.sin(ang),cross=Math.abs(dx*Math.sin(ang)-dy*Math.cos(ang));if(along>0&&along<range&&cross<e.r+8)hurt(e,d*1.4,id)}}cd=1.7-L*.12}
if(id==='flame'){let range=(105+L*12)*area*(A?1.3:1);for(let k=0;k<(A?16:7);k++){let ang=A?k*Math.PI/8:a+(k-3)*.13;beam(r.x,r.y,r.x+Math.cos(ang)*range,r.y+Math.sin(ang)*range,w.color,8)}for(let e of targets()){let ang=Math.atan2(e.y-r.y,e.x-r.x),delta=Math.atan2(Math.sin(ang-a),Math.cos(ang-a));if(Math.hypot(e.x-r.x,e.y-r.y)<range&&(A||Math.abs(delta)<.55))hurt(e,d*.35,id)}cd=.23}
if(id==='lightning'){let last={x:r.x,y:r.y},remaining=targets(),count=(2+L)*(A?2:1);for(let i=0;i<count&&remaining.length;i++){remaining.sort((a,b)=>Math.hypot(a.x-last.x,a.y-last.y)-Math.hypot(b.x-last.x,b.y-last.y));let e=remaining.shift();if(Math.hypot(e.x-last.x,e.y-last.y)>(i===0?500:190)*area)break;beam(last.x,last.y,e.x,e.y,w.color,3);hurt(e,d*.8,id);last=e}cd=2.4-L*.12}
...
function updateDrones(dt){if(!run.weapons.drone||run.ended)return;let r=run,st=weaponStats('drone'),drones=dronePositions();if(st.aw)r.hostile=r.hostile.filter(b=>!drones.some(d=>Math.hypot(b.x-d.x,b.y-d.y)<45));r.cooldowns.drone=(r.cooldowns.drone||0)-dt;if(r.cooldowns.drone>0)return;let fired=false;for(let d of drones){let nearby=combatTargets().filter(e=>Math.hypot(e.x-d.x,e.y-d.y)<=220*st.area+e.r);nearby.sort((a,b)=>Number(!!a.isChest)-Number(!!b.isChest)||Math.hypot(a.x-d.x,a.y-d.y)-Math.hypot(b.x-d.x,b.y-d.y));let target=nearby[0];if(!target)continue;beam(d.x,d.y,target.x,target.y,st.aw?'#ffe29a':'#75e9eb',st.aw?4:2);hurt(target,st.damage*.8,"drone");fired=true;if(r.ended)break}if(fired){r.cooldowns.drone=.65*st.cooldown;beep(680,.025,'sine',.006)}}
...
function collectDrop(chest){if(!run||run.ended||run.paused||chest.collected||!run.drops.includes(chest)||run.t<chest.readyAt||Math.hypot(chest.x-run.x,chest.y-run.y)>38)return false;chest.collected=true;let kind=chest.kind,text,color;if(kind==='gold'){run.goldChests++;let gold=Math.floor((30+run.stage*5)*(1+save.upgrades.gold*.1)*modeRules().gold);run.gold+=gold;text='골드 +'+gold+' G';color='#ffe093'}else if(kind==='magnet'){for(const orb of run.orbs)orb.attracted=true;text='자석 · 경험치 전체 회수';color='#92d9f2'}else{let hp=Math.min(run.maxHp-run.hp,Math.ceil(run.maxHp*.3));run.hp+=hp;text=hp>0?'회복 +'+hp:'회복 · 체력 가득 참';color='#a7efaa'}run.lootText.push({x:chest.x,y:chest.y-30,text,color,life:2.4});ring(chest.x,chest.y,65,color);beep(kind==='gold'?580:850,.15,'sine');return kind}
