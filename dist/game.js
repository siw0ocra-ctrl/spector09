...
function weaponTick(dt){updateDrones(dt);let r=run,targets=()=>combatTargets(),nearest=nearestTarget(targets(),r.x,r.y);for(const id of Object.keys(r.weapons)){if(id==='drone')continue;if(r.ended)return;r.cooldowns[id]=(r.cooldowns[id]||0)-dt;if(r.cooldowns[id]>0||!nearest)continue;let w=weapons.find(w=>w.id===id),st=weaponStats(id),a=Math.atan2(nearest.y-r.y,nearest.x-r.x),d=st.damage,L=st.level,A=st.aw,area=st.area,cd=1;r.angle=a;
