let simpleEffects=false;try{simpleEffects=localStorage.getItem('sector09-simple-effects')==='1'}catch{}
'use strict';
function start(){}
function end(){}
...
function weaponDetails(id){
 ...
 if(id==='nova')return `파동 ${damage(1.5)} 피해 · ${seconds(3.6-L*.2)}초 간격 · 반경 ${distance((90+L*15)*(A?1.6:1))}`;
 if(id==='drone')return `${A?6:Math.min(4,1+Math.floor(L/2))}기 · 기당 ${damage(.8)} 피해 · ${seconds(.65)}초 간격 · 드론 기준 사거리 ${distance(220)}${A?' · 근처 적 탄환 요격':''}`;
 if(id==='flame')return `${A?'전방위':'전방 부채꼴'} · 타격 ${damage(.35)} 피해 · ${seconds(.23)}초 간격 · 사거리 ${distance((105+L*12)*(A?1.3:1))}`;
