...
DB round-trip optimization (2026-09-18): ordinary compact cashout now makes three D1 calls (authenticate, receipt lookup, atomic write/read batch), compared with six before. SQL statement count falls from seven to five. The first mutation attempt reuses the authenticated row, guarded by the existing revision CAS. Conflicts reload and retry. The batch's final SELECT returns both the committed player and durable receipt; no locally predicted wallet is returned. Existing receipt fingerprints and transactional ranking/voucher writes remain intact. Tests cover stale snapshots, duplicate/different-ID concurrent cashouts, replay, and rollback when the final query fails. No schema or saved-data migration is needed.
